...
## Пауза 25.09.2026 20:47 — scope-fence: проект закреплён за dbrain_d82760f4 (агента нет), любая запись отбита; B-14/B-15 в очереди ab 20260925-204717-rodoslov-1a9e2509

- Почему: scope-fence: проект закреплён за dbrain_d82760f4 (агента нет), любая запись отбита; B-14/B-15 в очереди ab 20260925-204717-rodoslov-1a9e2509
- Цель ночи: прод-готовность по docs/ACCEPTANCE.md
- Неделя потрачена: неизвестно (резерв 10%), темп: behind
- Сессия: tmux · проект: /home/alexey-zhuykov/stack/agent-second-brain
- Что дальше: `/autopilot` — продолжить, `/autopilot off` — выключить

---
- Снято 25.09.2026 ~21:40: `dbrain_d82760f4` оказался боевым рантаймом бота, а не dev-сессией; задача ab отозвана, автопилот выключен владельцем, B-14/B-15 сделаны из rodoslov под `scope-fence bypass` владельца. B-14: `/process` и `VaultGit` коммитят только `vault/` (`-- .`), push убран, PRIVACY поправлен. B-15: `process.sh` — `chmod 700` каталога снимков и `umask 077` на tar; живой каталог 700. Проверено так: `uv run pytest -q` → 272 passed; `grep -c commit_and_push …/process.py` → 0; `stat -c %a ~/.dbrain/backups` → 700.
- Реестр scope-fence починен: `~/bin/session-registry` относит сессии `dbrain_*` (рантайм бота, `runtime.py:41`) к служебным (`scope_mode: none`, без `paths`); у `dbrain_d82760f4` снято владение репо; владелец `~/stack/agent-second-brain/` в `~/agent-bus/sessions.yml` — `rodoslov` (раньше проект падал на `44-srv` через `~/stack/`). Проверено так: эта строка записана из rodoslov без bypass.
